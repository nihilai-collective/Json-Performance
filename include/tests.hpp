...
namespace tests {
			...
			struct parse_test_struct {
				static size_t impl(jsonifier::jsonifier_core<>& parser_new, std::vector<test_data_type>& test_datas, std::vector<std::string>& json_data_in) {
					...
					for (size_t x = 0; x < test_datas.size(); ++x) {
						parser_new.parseJson<jsonifier::parse_options{ .partialRead = true, .minified = minified }>(test_datas[x], json_data_in[x]);
		...
		static auto run(std::string& json_data_in_pre) {
			...
			struct parse_test_struct {
				static size_t impl(jsonifier::jsonifier_core<>& parser_new, std::string& json_data_in) {
					test_data_type json_data_out;
					parser_new.parseJson<jsonifier::parse_options{ .partialRead = true, .knownOrder = true, .minified = minified }>(json_data_out,
						json_data_in);
			...
			parser.parseJson<jsonifier::parse_options{ .partialRead = true, .knownOrder = true, .minified = minified }>(json_data_out, json_data_in_pre);
	...
	std::string make_section02() {
		...
		result += "> Both libraries are parsing fully into the target data structures by first performing stage-1, then stage-2 parsing, and both of them are performing utf-8 "
	...
	void test_function() {
		...
		run_test_pair<double, "Double Test", true, json_libraries::jsonifier, json_libraries::simdjson>(newer_string);
		run_test_pair<uint64_t, "Uint64 Test", true, json_libraries::jsonifier, json_libraries::simdjson>(newer_string);
		run_test_pair<int64_t, "Int64 Test", true, json_libraries::jsonifier, json_libraries::simdjson>(newer_string);
		run_test_pair<bool, "Bool Test", true, json_libraries::jsonifier, json_libraries::simdjson>(newer_string);
		run_test_pair<std::string, "String Test", true, json_libraries::jsonifier, json_libraries::simdjson>(newer_string);
		run_test_pair<canada_message, "Canada Test", false, json_libraries::simdjson, json_libraries::jsonifier>(newer_string);
		run_test_pair<citm_catalog_message, "CitmCatalog Test", false, json_libraries::simdjson, json_libraries::jsonifier>(newer_string);
		run_test_pair<discord_message, "Discord Test", false, json_libraries::simdjson, json_libraries::jsonifier>(newer_string);
		run_test_pair<google_maps_response_message, "Google Maps Response Test", false, json_libraries::simdjson, json_libraries::jsonifier>(newer_string);
		run_test_pair<instruments_message, "Instruments Test", false, json_libraries::simdjson, json_libraries::jsonifier>(newer_string);
		run_test_pair<marine_ik_reverse, "Marine IK Reverse Test", false, json_libraries::simdjson, json_libraries::jsonifier>(newer_string);
		run_test_pair<marine_ik, "Marine IK Test", false, json_libraries::simdjson, json_libraries::jsonifier>(newer_string);
		run_test_pair<mesh_message, "Mesh Test", false, json_libraries::simdjson, json_libraries::jsonifier>(newer_string);
		run_test_pair<random_message, "Random Test", false, json_libraries::simdjson, json_libraries::jsonifier>(newer_string);
		run_test_pair<twitter_message, "Twitter Test", false, json_libraries::simdjson, json_libraries::jsonifier>(newer_string);
