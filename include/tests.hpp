...
namespace tests {
	...
	struct library_traits<test_types::parse, json_libraries::jsonifier, test_name_new, minified, test_data_type> {
		static auto run(const std::vector<std::string>& json_data_in) {
			...
			struct parse_test_struct {
				static size_t impl(jsonifier::jsonifier_core<>& parser_new, test_data_type& test_datas, const std::vector<std::string>& json_data_in) {
					size_t new_size{};
	...
	struct library_traits<test_types::parse, json_libraries::jsonifier, test_name_new, minified, test_data_type> {
		static auto run(std::string& json_data_in_pre) {
			...
			test_data_type json_data_out;
			parser.parseJson<jsonifier::parse_options{ .partialRead = partial_read, .knownOrder = known_order, .minified = minified, .validateUtf8 = true }>(
	...
	std::string make_section02() {
		...
		result += "-";
		result += benchmarksuite::system_info_data<benchmarksuite::benchmark_types::cpu>::instruction_set_name;
		...
		result += "](https://github.com/realtimechris/benchmarksuite/commit/";
		result += BNCH_SWT_COMMIT;
		result += ").\n  ";
		return result;
	}
	...
	void test_function() {
		...
		run_pod_tests<"Double Test", std::vector<double>, true>(newer_string);
		run_pod_tests<"Uint64 Test", std::vector<uint64_t>, true>(newer_string);
		run_pod_tests<"Int64 Test", std::vector<int64_t>, true>(newer_string);
		run_pod_tests<"Bool Test", std::vector<bool>, true>(newer_string);
		run_pod_tests<"String Test", std::vector<std::string>, true>(newer_string);
