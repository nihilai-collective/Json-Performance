...
namespace tests {
		...
		static auto run(std::string& json_data_in_pre) {
			...
			struct parse_test_struct {
				static size_t impl(jsonifier::jsonifier_core<>& parser_new, std::string& json_data_in) {
					...
					parser_new.parseJson<jsonifier::parse_options{ .partialRead = false, .knownOrder = known_order, .minified = minified }>(json_data_out,
						json_data_in);
			...
			std::string new_string;
			test_data_type json_data_out;
			parser.parseJson<jsonifier::parse_options{ .partialRead = false, .knownOrder = known_order, .minified = minified }>(json_data_out,
				json_data_in_pre);
	...
	void test_function() {
		...
		run_test_pair<marine_ik_reverse, "Marine IK Reverse Test", false, json_libraries::jsonifier, json_libraries::simdjson>(newer_string);
