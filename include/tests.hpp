		...
		static auto run(std::string& json_data_in_pre) {
			...
			struct parse_test_struct {
				static size_t impl(jsonifier::jsonifier_core<>& parser_new, std::string& json_data_in) {
					...
					parser_new.parseJson<jsonifier::parse_options{ .partialRead = false, .knownOrder = known_order, .minified = minified, .validateUtf8 = true }>(
			...
			std::string new_string;
			test_data_type json_data_out;
			parser.parseJson<jsonifier::parse_options{ .partialRead = false, .knownOrder = known_order, .minified = minified, .validateUtf8 = true }>(json_data_out,
				json_data_in_pre);
