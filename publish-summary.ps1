...
foreach ($projectKey in $projects.Keys) {
    ...
    foreach ($branch in $projectBranches) {
        ...
        if ($LASTEXITCODE -ne 0) {
            Write-Warning "  scp failed (exit code $LASTEXITCODE) fetching stats.json for '$projectKey' / '$branch' - skipping."
            continue
        }
...
if ($LASTEXITCODE -ne 0) {
    Write-Error "Uploading summary.json failed (exit code $LASTEXITCODE)."
    exit 1
}
