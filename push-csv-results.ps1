...
param(
    ...
    [Parameter(Mandatory = $false)]
    [ValidateSet("main", "stage-1-parsing-main", "stage-1-parsing-avx512-main", "full-parsing-utf8-main", "correctness-validation-main", "correctness-validation-avx512-main", "scalar-vs-simd-main")]
    [string]$Branch,
...
function Invoke-Remote {
    ...
    if ($LASTEXITCODE -ne 0) {
        Write-Error "$Description failed (exit code $LASTEXITCODE) - aborting."
        exit 1
    }
}
...
if (-not $Branch) {
    ...
    $validBranches = @("main", "stage-1-parsing-main", "stage-1-parsing-avx512-main", "full-parsing-utf8-main", "correctness-validation-main", "correctness-validation-avx512-main", "scalar-vs-simd-main")
...
if ($Branch -in @("correctness-validation-main", "correctness-validation-avx512-main")) {

    Write-Host "Pushing $Branch .md files..."

    $correctnessRemoteDir = "$RemoteRoot/$Project/$Branch"
    $missing = @()
    $foundFiles = @()
    ...
    Invoke-Remote "Creating remote directory '$correctnessRemoteDir'" { ssh -i $KeyPath $RemoteHost "mkdir -p '$correctnessRemoteDir'" }

    foreach ($filePath in $foundFiles) {
        $fileName = Split-Path $filePath -Leaf
        Write-Host "  Uploading $fileName..."
        Invoke-Remote "Uploading $fileName" { scp -i $KeyPath $filePath "${RemoteHost}:${correctnessRemoteDir}/$fileName" }
    }
    ...
    Invoke-Remote "Creating remote directory '$remoteDir'" { ssh -i $KeyPath $RemoteHost "mkdir -p '$remoteDir'" }

    Write-Host "Uploading archive to server..."
    $remoteTarPath = "/tmp/$tarName"
    Invoke-Remote "Uploading archive" { scp -i $KeyPath $tarPath "${RemoteHost}:${remoteTarPath}" }

    Write-Host "Extracting on server into '$remoteDir'..."
    Invoke-Remote "Extracting archive on server" { ssh -i $KeyPath $RemoteHost "rm -rf '$remoteDir' && mkdir -p '$remoteDir' && tar -xzf '$remoteTarPath' -C '$remoteDir'" }
    Invoke-Remote "Removing remote archive" { ssh -i $KeyPath $RemoteHost "rm -f '$remoteTarPath'" }
