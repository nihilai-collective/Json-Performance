...
function Get-JsonifierTestNameAndParams {
    ...
    if ($FileBaseName -match '^(.+?)\s+(Read|Write)(\s+Pre-Alloc)?$') {
        $baseName = $Matches[1]
        $operation = $Matches[2].ToLowerInvariant()
        $isPreAlloc = [bool]$Matches[3]

        $params = [ordered]@{ operation = $operation }
        if ($isPreAlloc) {
            $params.preAlloc = $true
        }

        $displayName = if ($isPreAlloc) { "$baseName (Pre-Alloc)" } else { $baseName }
